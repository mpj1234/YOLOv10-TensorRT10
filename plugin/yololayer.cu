...
namespace nvinfer1 {
    ...
    size_t YoloLayerPlugin::getSerializationSize() const TRT_NOEXCEPT {
        return sizeof(mClassCount) + sizeof(mThreadCount) +
               sizeof(mYoloV10netHeight) + sizeof(mYoloV10NetWidth) + sizeof(mMaxOutObject) + sizeof(mStridesLength) +
    ...
    nvinfer1::IPluginV2IOExt *YoloLayerPlugin::clone() const TRT_NOEXCEPT {
        ...
        YoloLayerPlugin *p =
                new YoloLayerPlugin(mClassCount, mYoloV10NetWidth,mYoloV10netHeight,
                                    mMaxOutObject, mStrides, mStridesLength);
    ...
    int YoloLayerPlugin::enqueue(int batchSize, const void *TRT_CONST_ENQUEUE *inputs, void *const *outputs,
        ...
        forwardGpu((const float *const *) inputs, (float *) outputs[0], stream, mYoloV10netHeight, mYoloV10NetWidth,
                   batchSize);
        return 0;
    }
    ...
    void
    YoloLayerPlugin::forwardGpu(const float *const *inputs, float *output, cudaStream_t stream, int mYoloV10netHeight,
        ...
        for (unsigned int i = 0; i < maxGrids; i++) {
            ...
            CalDetection<<<(numElem + mThreadCount - 1) / mThreadCount, mThreadCount, 0, stream>>>(
                    inputs[i], output, numElem, mMaxOutObject, grid_h, grid_w, stride,
                    mClassCount, outputElem);
        }
    ...
    IPluginV2IOExt *YoloPluginCreator::createPlugin(const char *name, const PluginFieldCollection *fc) TRT_NOEXCEPT {
        ...
        YoloLayerPlugin *obj =
                new YoloLayerPlugin(class_count, input_w, input_h,
                                    max_output_object_count, px_arry, px_arry_length);
